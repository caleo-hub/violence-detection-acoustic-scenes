...
class VSD_DatasetGenerator:
    ...
    def get_filenames(self):
        ...
        filenames_filtered = list(
            filter(
                lambda x: any(class_name in x for class_name in self.class_names),
                filenames,
            )
        )
    ...
    def get_movie_original_annotations(self):
        ...
        for movie_name in movie_names:
            movies_filename[movie_name] = list(
                filter(lambda x: movie_name in x, self.txt_files)
            )
            ...
            for filename in filenames:
                ...
                with open(file_path, "r") as file:
                    for line in file:
                        ...
                        movie_annotations[movie].append(
                            (start_time, end_time, annotation, tag, duration)
                        )
    ...
    def remove_nothing_annotations(self, movie_annotations):
        ...
        for movie, annotations in movie_annotations.items():
            no_nothing_annotations[movie] = [
                annotation for annotation in annotations if annotation[2] != "(nothing)"
            ]
    ...
    def remove_inner_annotations(self, annotations):
        ...
        for i, current_annotation in enumerate(annotations):
            ...
            for j, other_annotation in enumerate(annotations):
                if (
                    i != j
                    and current_annotation[0] >= other_annotation[0]
                    and current_annotation[1] <= other_annotation[1]
                ):
                    is_inner = True
                    break
            if not is_inner:
    ...
    def find_intersections(self, temp_annotations):
        ...
        for i, current_annotation in enumerate(temp_annotations):
            ...
            for j, other_annotation in enumerate(temp_annotations):
                if (
                    i != j
                    and current_annotation[1] > other_annotation[0]
                    and current_annotation[0] < other_annotation[1]
                ):
    ...
    def add_nothing_annotations(self, annotations):
        ...
        complete_annotations.append(
            (0.0, timeline[0][0], "(nothing)", "", timeline[0][0])
        )
        for i in range(len(timeline) - 1):
            ...
            if current_annotation[1] < next_annotation[0]:
                ...
                complete_annotations.append(
                    (start_time, end_time, "(nothing)", "", duration)
                )
