...
class AudioDatasetIntegrator:
    ...
    def get_audio_dataset(self, path, file_output='src/h5_files/audios.h5'):
        
        audio_info_df = self.organize_dataframe(self.extract_audio_info(folder=path))
        annotation = self.annotations_loader(audio_info_df)
        self.audio_loader(audio_info_df, annotation, h5_filepath=file_output)
        
        return {'audio': 'audios.h5',
    ...
    def annotations_loader(self, audio_info_df):
        ...
        annotation_list = []
        annotation_list = audio_info_df['Anotação'].apply(
        ...
        with h5py.File(h5_filepath, 'w') as f:
            for index, row in audio_info_df.iterrows():
                try:
                    ...
                    # Cria um dataset no arquivo h5 para armazenar o áudio
                    audio_ds = f.create_dataset(str(index), data=padded_audio)
                    audio_ds.attrs["annotation"] = annotation[index]
